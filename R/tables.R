...
.table_stats <- function(objects, colored_lines = list(1:3,4:7,8:10,11:13,14:16,17:26), lines_colors = c("gray", "gray", "red","green", "blue","gray"), col = 8, align_to_shortest = TRUE, sharpe_scale = 365, verbose = TRUE) {
  ...
  get_trades_xts <- function(obj) {
    if (!is.null(obj$trades) && xts::is.xts(obj$trades)) return(obj$trades)
    for (nm in c("txn", "transactions", "Trades", "TXN")) {
      if (!is.null(obj[[nm]]) && xts::is.xts(obj[[nm]])) return(obj[[nm]])
    }
  ...
  get_stats_df <- function(obj) {
    if (!is.null(obj$stats)) return(as.data.frame(obj$stats))
    st_attr <- attr(obj, "stats", exact = TRUE)
    if (!is.null(st_attr)) return(as.data.frame(st_attr))
    NULL
  }

  # Detect columns in trades (case-insensitive)
  detect_trade_cols <- function(tr) {
    cn <- colnames(tr); lc <- tolower(cn)
    pick <- function(cands) {
      for (nm in cands) {
        w <- which(lc == tolower(nm))
        if (length(w) == 1) return(cn[w])
      }
      NULL
    }
    list(
  ...
  get_qty_full <- function(tr, cols, tol = 1e-10) {
    ...
    if (!is.null(cols$value) && !is.null(cols$price)) {
      val <- as.numeric(tr[, cols$value]); val[is.na(val)] <- 0
      prc <- as.numeric(tr[, cols$price]); prc[is.na(prc) | abs(prc) < tol] <- NA_real_
      q <- val / prc
      q[is.na(q)] <- 0
      return(q)
    }
  ...
  get_symbol <- function(obj, tr) {
    ...
    if (!is.null(st) && "Symbol" %in% names(st)) {
      sym <- unique(as.character(st$Symbol)); if (length(sym) > 0) return(sym[1])
    }
    sattr <- attr(tr, "symbol"); if (!is.null(sattr)) return(as.character(sattr))
    cn <- colnames(tr); if (!is.null(cn) && length(cn) > 0) return(as.character(cn[1]))
    "Symbol"
  }
  get_portfolio <- function(obj) {
    st <- get_stats_df(obj)
    if (!is.null(st) && "Portfolio" %in% names(st)) {
      pf <- unique(as.character(st$Portfolio)); if (length(pf) > 0) return(pf[1])
    }
    NA_character_
  }
  ...
  grab_constant_fields <- function(obj) {
    st <- get_stats_df(obj); if (is.null(st)) return(NULL)
  ...
  build_trade_segments_full <- function(tr, qty_full, pl_full) {
    n <- length(qty_full)
    if (n == 0) return(data.frame(start_idx=integer(0), end_idx=integer(0),
    ...
    if (length(segs) == 0) {
      return(data.frame(start_idx=integer(0), end_idx=integer(0),
                        start_time=as.POSIXct(character(0)),
                        end_time=as.POSIXct(character(0)),
                        pl=numeric(0)))
    }
